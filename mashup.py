...
def convert_to_audio():
    for filename in os.listdir():
        if filename.endswith(".3gpp"):
            ...
            audio = video.audio
            audio.write_audiofile(os.path.join(filename.split(".")[0] + ".mp3"))
...
def cut_audio(seconds):
    for audio_file in os.listdir():
        if audio_file.endswith(".mp3"):
            ...
            cut_audio_path = os.path.join(audio_file)
...
#             audio_clips.append(AudioFileClip(os.path.join(filename)))
#     final_audio = concatenate_audioclips(audio_clips)
#     final_audio.write_audiofile()
#     print("Merged all audio files in the folder successfully")
def merge_audios():
    audio_files = [AudioFileClip(file) for file in os.listdir(os.getcwd()) if file.endswith('.mp3')]
